...
/**
 * Ordre d'affichage des distinctions (priorité automatique, aucun choix manuel) :
 *   1. distinction annuelle la plus récente
 *   2. plus haut palier d'accomplissement (100 > 50 > 25 > 10)
 *   3. fonction actuelle (Team Leader)
 *   4. le reste
 */
export function sortBadges(list) {
  const rank = (b) => {
    if (b.scope === "annuel")
      return [0, -(Number(b.annee) || 0), -new Date(b.obtenu_le || 0).getTime()];
    if (b.nature === "accomplissement") {
      const n = parseInt((String(b.code).match(/_(\d+)$/) || [])[1], 10) || 0;
      return [1, -n, 0];
    }
    if (b.nature === "fonction") return [2, 0, 0];
    return [3, 0, 0];
  };
